...
# 项目根目录 = agent/  (src/adapters/run_api_server.py → .parent.parent.parent)
...
def main():
    ...
    persona_name = pipeline.personality.name
    model_name = args.model_name or persona_name

    logger.info(
        f"Pipeline 初始化完成: persona={persona_name}, "
        f"model={pipeline.llm_client.model}"
    )

    # 2. 创建 FastAPI 应用（传入安全配置）
    from src.adapters.openai_adapter import create_app

    security_config = app_cfg.security
    app = create_app(pipeline, model_name=model_name, security_config=security_config)
