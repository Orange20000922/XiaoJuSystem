...
class InMemoryRateLimiter:
    ...
    def cleanup(self, max_age: float = 120.0):
        ...
        cutoff = now - max_age
        with self._lock:
            stale = [key for key, ts in self._windows.items() if not ts or ts[-1] < cutoff]
            for key in stale:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self._config.enabled:
            ...
            if key not in self._api_keys:
                ...
                if triggered:
                    message += ". IP has been temporarily banned."
                return _error_response(401, message, "auth_error", "invalid_api_key")
            ...
            if not self._rate_limiter.is_allowed(
                f"key:{key}",
                self._config.rate_limit_per_key_per_minute,
            ):
                return _error_response(
                    429,
