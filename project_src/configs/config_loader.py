...
def load_memory_config(cfg: dict) -> MemoryConfig:
    ...
    return MemoryConfig(
        ...
        relevance_threshold=m.get("relevance_threshold", 0.50),
        memory_recall_overfetch=m.get("memory_recall_overfetch", 3),
        l2_relevance_threshold=m.get("l2_relevance_threshold", 0.40),
        l2_history_threshold=m.get("l2_history_threshold", 0.30),
        l3_recent_dialog_threshold=m.get("l3_recent_dialog_threshold", 0.35),
        l3_recent_dialog_history_threshold=m.get(
            "l3_recent_dialog_history_threshold",
            0.20,
        ),
        l3_fact_threshold=m.get("l3_fact_threshold", 0.50),
        l3_visual_threshold=m.get("l3_visual_threshold", 0.50),
        l3_default_threshold=m.get("l3_default_threshold", 0.50),
        l4_relevance_threshold=m.get("l4_relevance_threshold", 0.70),
        lifecycle_store_path=m.get("lifecycle_store_path") or None,
        enable_forgetting=m.get("enable_forgetting", False),
        forgetting_interval_hours=m.get("forgetting_interval_hours", 24),
        memory_count_trigger=m.get("memory_count_trigger", 5000),
        min_retention_days=m.get("min_retention_days", 1.0),
        physical_deletion_delay_days=m.get("physical_deletion_delay_days", 7.0),
        lambda_l2=m.get("lambda_l2", 0.30),
        lambda_l3=m.get("lambda_l3", 0.10),
        base_weight_l2=m.get("base_weight_l2", 0.70),
        base_weight_l3=m.get("base_weight_l3", 1.00),
        alpha_recall=m.get("alpha_recall", 0.20),
        mood_coeff_v=m.get("mood_coeff_v", 0.10),
        mood_coeff_a=m.get("mood_coeff_a", 0.10),
        encoding_intensity_coeff=m.get("encoding_intensity_coeff", 0.30),
        encoding_arousal_coeff=m.get("encoding_arousal_coeff", 0.20),
        configured_W_ref=m.get("configured_W_ref", 1.50),
        max_prune_prob=m.get("max_prune_prob", 0.80),
        depth_bias=m.get("depth_bias", 0.05),
        random_jitter_sigma=m.get("random_jitter_sigma", 0.10),
        random_seed=m.get("random_seed"),
        recovery_enabled=m.get("recovery_enabled", True),
        recovery_threshold=m.get("recovery_threshold", 0.70),
        recovery_recent_dialog_threshold=m.get("recovery_recent_dialog_threshold"),
        recovery_rate_limit_per_day=m.get("recovery_rate_limit_per_day", 10),
        hard_delete_enabled=m.get("hard_delete_enabled", False),
    )
