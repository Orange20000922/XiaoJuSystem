...
# 项目根目录 (src/server/qq/run_qq_bot.py → .parent.parent.parent.parent)
...
def _start_single_persona(args):
    """单人格模式（通过 PersonaScheduler 统一管理）"""
    from configs.config_loader import AppConfig
    from configs.model_config import SchedulerConfig
    from src.core_engine import PersonaScheduler, SharedInfra
    from src.core_engine.api import MultiRuntime
    ...
    adapter.multi_runtime = MultiRuntime(scheduler)
...
def _start_multi_persona(args):
    """多人格模式（从 scheduler_config.json 加载）"""
    ...
    for entry in entries:
        ...
        exact = {item for item in contexts if "*" not in item and "?" not in item}
        patterns = [item for item in contexts if "*" in item or "?" in item]
...
def main():
    ...
    parser.add_argument("--config", default=None, help="config.json 路径（单人格模式）")
    parser.add_argument(
        "--scheduler",
        default=None,
        help="scheduler_config.json 路径（多人格模式）",
    )
    ...
    try:
        ...
        logger.critical(f"未捕获异常: {exc}", exc_info=True)
