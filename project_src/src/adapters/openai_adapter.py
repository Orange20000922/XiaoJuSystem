...
将 NeuroLikeSystem pipeline 包装为 OpenAI `/v1/chat/completions` 兼容的 HTTP 服务，
...
def _build_response(completion_id: str, model: str, content: str,
...
def _build_chunk(completion_id: str, model: str, delta: Dict,
...
def create_app(pipeline, model_name: str = "educational-agent",
               security_config: Optional[SecurityConfig] = None) -> FastAPI:
    """
    创建 FastAPI 应用。

    Args:
        pipeline: NeuroLikePipeline 实例（已初始化）
    ...
    async def chat_completions(req: ChatCompletionRequest):
        ...
        try:
            ...
            logger.error(f"Pipeline 调用失败: {e}", exc_info=True)
            return JSONResponse(
                status_code=500,
        ...
        if not req.stream:
            return JSONResponse(
                content=_build_response(completion_id, model_name, response_text),
            )

        # 5. 流式响应（SSE）
        async def stream_generator() -> AsyncGenerator[str, None]:
            yield _build_chunk(completion_id, model_name, {"role": "assistant"})
            for char in response_text:
                yield _build_chunk(completion_id, model_name, {"content": char})
                await asyncio.sleep(0.02)
            yield _build_chunk(completion_id, model_name, {}, finish_reason="stop")
            yield "data: [DONE]\n\n"
    ...
    async def health():
        return {
            "status": "ok",
            "model": model_name,
            "persona": pipeline.personality.name,
        }

    # ── 安全中间件 ──────────────────────────────────────────────────

    from src.adapters.security import apply_security
