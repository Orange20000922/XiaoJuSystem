...
from configs.model_config import ImageConfig, QQBotConfig
from src.agent.agent_loop import AgentEvent, AgentLoop
from src.core_engine.api import EventRuntime, MultiRuntime
from src.core_engine.runtime_types import ChatMode
from src.logger import logger
from src.media.image_utils import PILLOW_AVAILABLE, ImageResult, process_image_url
...
def is_at_me(cq_codes: list, bot_qq: int) -> bool:
    """检查 CQ 码中是否有 @机器人"""
    return any(c["type"] == "at" and str(c.get("qq")) == str(bot_qq) for c in cq_codes)
...
def extract_image_urls(cq_codes: list, max_images: int = 5) -> List[str]:
    ...
    for code in cq_codes:
        if code["type"] == "image" and "url" in code:
            # CQ 码中的 URL 可能包含 HTML 转义（如 &amp; → &）
            urls.append(html.unescape(code["url"]))
...
class QQBotAdapter:
    ...
    def __init__(self, config: QQBotConfig, image_config: Optional[ImageConfig] = None):
        ...
        self.runtime: Optional[EventRuntime] = None
        self.multi_runtime: Optional[MultiRuntime] = None
    ...
    def _single_runtime(self) -> Optional[EventRuntime]:
        if self.runtime is not None:
            return self.runtime
        if self.agent_loop is not None:
            return EventRuntime(self.agent_loop)
        return None

    def _scheduler_runtime(self) -> Optional[MultiRuntime]:
        if self.multi_runtime is not None:
            return self.multi_runtime
        if self.scheduler is not None:
            return MultiRuntime(self.scheduler)
        return None
        ...
        try:
            async for raw in websocket:
                try:
                    ...
                    logger.error(f"处理事件异常: {exc}", exc_info=True)
        except ConnectionClosed as exc:
            logger.info(f"NapCat 断开连接: code={exc.code} reason={exc.reason}")
        finally:
    ...
    async def _on_message(self, event: dict):
        ...
        multi_runtime = self._scheduler_runtime()
        if multi_runtime:
            if not multi_runtime.push(event):
                logger.warning(f"无 persona 处理 context_id={context_id}")
        else:
            single_runtime = self._single_runtime()
            if single_runtime:
                single_runtime.push(event)
        ...
        def callback(text: str, reply_context: dict):
            ...
            for chunk in chunks:
                ...
                try:
                    ...
                    adapter.notify_owner_sync(f"[异常] 发送消息失败: {exc}")
    ...
    async def _download_images_async(self, urls: List[str]) -> List[ImageResult]:
        ...
        for index, result in enumerate(results):
            if isinstance(result, ImageResult):
                ...
                logger.warning(f"图片下载失败: {urls[index][:80]} — {result}")
    ...
    async def _cmd_help(self, event: dict):
        ...
        lines = ["小橘可用命令:"]
        for key, (desc, admin_only) in COMMANDS.items():
    ...
    async def _cmd_status(self, event: dict):
        ...
        multi_runtime = self._scheduler_runtime()
        if multi_runtime:
            report = multi_runtime.get_health_report()
            status_lines = [f"调度器状态 ({len(report)} 个 persona):"]
            for name, info in report.items():
                alive = "运行中" if info.running else "已停止"
                uptime = info.uptime_seconds or 0
                if uptime < 60:
        ...
        single_runtime = self._single_runtime()
        if single_runtime:
            status = single_runtime.get_status()
            state = status.proactive_state or "unknown"
            status_lines.append(f"  主动性状态: {state}")
            idle = status.idle_seconds or 0
            if idle < 60:
                status_lines.append(f"  空闲时间: {idle}秒")
            else:
                status_lines.append(f"  空闲时间: {idle // 60}分钟")
            status_lines.append(f"  工作记忆轮数: {status.working_memory_turns}")

            att_status = status.attention or {}
            if att_status:
                status_lines.append(f"  注意力焦点用户: {att_status.get('focused_users', 0)}")
                status_lines.append(f"  上下文活跃用户: {att_status.get('context_users', 0)}")
                if att_status.get("cooldown_active"):
                    status_lines.append("  回复冷却: 进行中")
                elif att_status.get("last_reply_ago") is not None:
    ...
    async def _cmd_clear(self, event: dict):
        ...
        if multi_runtime:
            ...
            result = multi_runtime.clear_context(ctx)
            if result.cleared_turns > 0:
                await self._send_reply(
                    event,
                    f"[{result.persona_name}] 已清空 {result.cleared_turns} 条工作记忆~",
                )
            else:
        ...
        if single_runtime:
            ...
            if msg_type == "group":
                ...
                ctx = f"private_{user_id}"
            result = single_runtime.clear_context(ctx)
            if result.cleared_turns > 0:
                await self._send_reply(event, f"已清空 {result.cleared_turns} 条工作记忆~")
            else:
                await self._send_reply(event, "没有可清空的对话历史")
        else:
            await self._send_reply(event, "没有可清空的对话历史")
    ...
    async def _cmd_exit(self, event: dict):
        ...
        multi_runtime = self._scheduler_runtime()
        if multi_runtime:
            multi_runtime.shutdown()
            logger.info("PersonaScheduler 已停止")
        else:
            single_runtime = self._single_runtime()
            if single_runtime:
                single_runtime.shutdown()
    ...
    async def notify_owner(self, text: str):
        ...
        try:
            ...
            logger.warning(f"通知 owner 失败: {exc}")
    ...
    def notify_owner_sync(self, text: str):
        ...
        try:
            ...
            logger.warning(f"同步通知 owner 失败: {exc}")
